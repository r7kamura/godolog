...
export function listArticles() {
  ...
  return articles.sort(({ name: a }, { name: b }) => {
    if (a < b) {
      return 1;
    } else if (a > b) {
      return -1;
...
function documentToArticle(document) {
  ...
  const dateString = documentNameToDateString(document.name);
 ...
 * @returns {string|null}
 */
function documentNameToDateString(name) {
...
function listDocuments() {
  ...
  return fileNames
    ...
    .filter((document) => {
      return !!documentNameToDateString(document.name);
    });
}
